...
class MyGraph:
    ...
    """
    def __init__(self, title, nblin=1, nbcol=1, is_mono=False, figsize=(12,8), frameon=True, polar=False, gridspec=False):
        sns.set_style("white")
        ...
        if (nblin == 1) & (nbcol == 1):
            ...
            if gridspec:
                self.__init_graph_gridspec(title=title, nblin=nblin, nbcol=nbcol)
            else:
    ...
    def __init_graph_gridspec(self, title, nblin, nbcol):
        self.fig.subplots_adjust(wspace=0.25, hspace=0.25)
    ...
    def graph_compar_distrib_normal(self, data, label_data, bins=20, color_distrib=None, alpha_distrib=1, linewidth=1.5, color_loi_normale=None, color_mean=None, show_median=False, color_median=None, subtitle=None, legend=True, with_grid=None, grid_style=":", multi_index=1):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style)
    ...
    def graph_sarimax_analysis(self, my_serie, my_index, centered=False, lags=40):
        ...
        sm.graphics.tsa.plot_acf(my_serie, lags=40, ax=self.ax[1])
        ...
        sm.graphics.tsa.plot_pacf(my_serie, lags=40, ax=self.ax[2])
