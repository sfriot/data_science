...
class ACP:
    ...
    def __init__(self, x, n_components, with_reduction=True):
        try:
            ...
            self.df_valeurs_propres = None
            self.df_choix_n_components = None
            self.df_centroids_projected = None
            self.df_variables_correlation = None
            self.calculate_acp()
    ...
    """
    Calculates the 3 output dataframes : df_variables_projected, df_data_projected, df_valeurs_propres
    """
    def calculate_acp(self):
        correct_valeurs_propres = (self.data_size - 1.0) / self.data_size
        ...
        self.df_valeurs_propres = pd.DataFrame(data={"explained_variance" : correct_valeurs_propres * self.pca.explained_variance_, \
            "explained_variance_ratio" : self.pca.explained_variance_ratio_}, index=["F{}".format(i+1) for i in np.arange(self.n_components)])
        self.df_choix_n_components = self.df_valeurs_propres[["explained_variance"]].copy
        ...
        self.df_choix_n_components["seuils_batons_brises"] = seuils_batons_brises
        self.df_centroids_projected = None
        
        
    def calcul_variables_interpretation(self):
        # correlation des variables avec les axes
        p = len(self.data_variables)
        corvar = np.zeros((p, p))
        sqrt_eigval = np.sqrt(self.df_valeurs_propres.explained_variance)
        for k in np.arange(p):
            corvar[:,k] = self.pca.components_[k,:] * sqrt_eigval[k]
        # qualité de représentation des variables COS²
        cos2var = corvar**2
        # contribution relative des variables aux axes CTR
        ctrvar = cos2var
        for k in np.arange(p):
            ctrvar[:,k] = ctrvar[:,k] / self.df_valeurs_propres.explained_variance[k]
        self.df_variables_correlation = pd.DataFrame(data=corvar, index=self.data_variables, columns=["COR F{}".format(i+1) for i in np.arange(self.n_components)])
        self.df_variables_qualite_representation = pd.DataFrame(data=cos2var, index=self.data_variables, columns=["COS² F{}".format(i+1) for i in np.arange(self.n_components)])
        self.df_variables_contribution = pd.DataFrame(data=ctrvar, index=self.data_variables, columns=["CTR F{}".format(i+1) for i in np.arange(self.n_components)])
        
    def calcul_individus_interpretation(self):
        # contrib des individus à l'inertie totale (carrés des distances à l'origine)
        carre_distances = np.sum(self.data_scaled**2, axis=1)
        # qualité de représentation des individus (cos2) sur chaque axe
        cos2 = self.data_projected**2
        for j in np.arange(self.n_components):
            cos2[:,j] = cos2[:,j] / carre_distances
        # contribution des individus aux axes CTR
        ctr = self.data_projected**2
        for j in np.arange(self.n_components):
            ctr[:,j] = ctr[:,j] / (self.data_size * self.df_valeurs_propres.explained_variance[j])
        self.df_individus_inertie = pd.DataFrame(data=carre_distances, index=self.data_names, columns=["contrib_inertie"])
        self.df_individus_qualite_representation = pd.DataFrame(data=cos2, index=self.data_names, columns=["COS² F{}".format(i+1) for i in np.arange(self.n_components)])
        self.df_individus_contribution = pd.DataFrame(data=ctr, index=self.data_names, columns=["CTR F{}".format(i+1) for i in np.arange(self.n_components)])
