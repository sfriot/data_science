...
def calculate_lorenz_gini(listdata):
...
class MyGraph:
    ...
    def add_histogramme(self, values, labels=None, bins=None, density=True, stacked=False, bar_width_percent=None, color=None, alpha=1, subtitle=None, legend=False, with_grid='both', grid_style="-", multi_index=1, twinx=False):
    ...
    def add_plot(self, x_values, y_values, label, marker=None, linestyle="-", color=None, alpha=1, linewidth=1.5, subtitle=None, legend=False, with_grid=None, grid_style="-", multi_index=1, twinx=False):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style, twinx)
        self.objets.append(self.ax[multi_index-1].plot(x_values, y_values, label=label, marker=marker, linestyle=linestyle, color=color, alpha=alpha, linewidth=linewidth))
    ...
    def add_boxplot(self, values, cat_labels, color=None, outliers=True, mult_iq=1.5, means=False, width=0.7, vertical=True, alpha=1, subtitle=None, legend=False, grid_style=":", multi_index=1):
        if vertical:
            with_grid = 'y'
        else:
            with_grid = 'x'
    ...
    def add_texts_boxplot(self, text, x, y, multi_index=1):
    ...
    """
    def add_pairplot(self, data, hue):
        if len(self.ax) == 1:
            self.ax[0] = sns.pairplot(data, hue=hue)        
        
    def graph_concentration(self, lorenz, gini, x_label, y_label, subtitle=None, with_grid='both', grid_style=":", multi_index=1):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style)
    ...
    def graph_compar_distrib_normal(self, data, min_bin, max_bin, label_data, color_distrib=None, alpha_distrib=1, linewidth=1.5, color_loi_normale=None, color_mean=None, show_median=False, color_median=None, subtitle=None, with_grid=None, grid_style=":", multi_index=1):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style)
        self.objets.append(self.ax[multi_index-1].hist(data, bins=range(min_bin, max_bin), density=True, label=label_data, color=color_distrib, alpha=alpha_distrib))
        self.objets_desc.append("Histogramme des données - axe {}".format(multi_index-1))
        x_theo = np.arange(min_bin, max_bin, 0.005*(max_bin-min_bin))
        data_mean = data.mean()
    ...
    def add_line(self, coord, vertical=True, label=None, line_min=0, line_max=1, linestyle="-", color=None, alpha=1, linewidth=1.5, multi_index=1):
        if vertical:
    ...
    def add_rectangle(self, coord_min, coord_max, vertical=True, label=None, rect_min=0, rect_max=1, linestyle="-", color=None, alpha=1, linewidth=1.5, multi_index=1):
        if vertical:
            self.objets.append(self.ax[multi_index-1].axvspan(xmin=coord_min, xmax=coord_max, ymin=rect_min, ymax=rect_max, label=label, linestyle=linestyle, color=color, alpha=alpha, linewidth=linewidth))
            self.objets_desc.append("Rectangle vertical - axe {}".format(multi_index-1))
        else:
            self.objets.append(self.ax[multi_index-1].axhspan(ymin=coord_min, ymax=coord_max, xmin=rect_min, xmax=rect_max, label=label, linestyle=linestyle, color=color, alpha=alpha, linewidth=linewidth))
            self.objets_desc.append("Rectangle horizontal - axe {}".format(multi_index-1))
        
    def set_axe(self, axe, label=None, label_position=(0.5,0.5), tick_min=None, tick_max=None, tick_step=None, tick_labels=None, tick_labels_format=None, tick_dash=False, color=None, rotation=None, ha=None, multi_index=1, twinx=False):
        """
        axe : 'x' ou 'y'
        ...
        if axe == 'x':
            ...
            if (color is not None) | (rotation is not None) | (ha is not None):
    ...
    """
    def set_legend(self, loc='best', bbox_to_anchor=None, ncol=1, title=None, multi_index=1):
        if multi_index == 0:
