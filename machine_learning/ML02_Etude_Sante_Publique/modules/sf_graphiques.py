...
def set_colors(is_mono, color_palette=None):
...
class MyGraph:
    ...
    """
    def __init__(self, title, y_title=1.025, nblin=1, nbcol=1, is_mono=False, color_palette=None, figsize=(12,8), wspace=0.25, hspace=0.25, frameon=True, polar=False, gridspec=False):
        sns.set_style("white")
        if (nblin > 1) & (figsize == (12,8)):
            figsize = (12, 6 * nblin)
        self.mult_param = mydefault_plt_parameters(figsize=figsize)
        self.liste_couleurs = set_colors(is_mono, color_palette)
        self.polar = polar
    ...
    def __add_bar(self, orientation, x_values, y_height, with_grid, grid_style, label=None, y_bottom=None, bar_width=0.8, color=None, alpha=1, subtitle=None, legend=False, multi_index=1, twinx=False):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style, twinx)
    ...
    def add_barv(self, x_values, y_height, label=None, y_bottom=None, bar_width=0.8, color=None, alpha=1, subtitle=None, legend=False, with_grid='y', grid_style=":", multi_index=1, twinx=False):
        orientation = 'vertical'
        self.__add_bar(orientation, x_values, y_height, with_grid, grid_style, label, y_bottom, bar_width, color, alpha, subtitle, legend, multi_index, twinx)
    
    def add_barh(self, x_values, y_height, label=None, y_bottom=None, bar_width=0.8, color=None, alpha=1, subtitle=None, legend=False, with_grid='x', grid_style=":", multi_index=1, twinx=False):
    ...
    def add_scatter(self, x_values, y_values, label=None, marker=None, size=None, color=None, alpha=1, subtitle=None, legend=False, with_grid='both', grid_style="-", multi_index=1, twinx=False):
        multi_index = self.__add_grid_twinx(multi_index, with_grid, grid_style, twinx)
    ...
    def add_pie(self, values, labels, explodes=None, colors=None, autopct='%1.1f%%', startangle=90, shadow=False, counterclock=True, subtitle=None, legend=False, multi_index=1):
    ...
    def add_text(self, x_coord, y_coord, text, ha='center', va='top', color='black', fontsize=None, rotation=0, alpha=1, backgroundcolor=(0.95, 0.95, 0.95), backgroundalpha=1, multi_index=1):
        if multi_index == 0:
            ...
            self.objets.append(self.ax[multi_index-1].text(x_coord, y_coord, text, horizontalalignment=ha, verticalalignment=va, color=color, backgroundcolor=backgroundcolor, bbox={'alpha':backgroundalpha}))
...
def graph_distribution_variable(data, nom_variable, label_x_boxplot, title=None, color_palette=None, figsize=(12,8)):
    if title is None:
    ...
    graph.add_boxplot([data], [nom_variable], means=True, vertical=False, subtitle="Boxplot", multi_index=1)
    graph.set_axe_x(label=label_x_boxplot, multi_index=1)
    graph.graph_droite_henry(data, subtitle="Droite de Henry", multi_index=2)
    graph.graph_compar_distrib_normal(data, nom_variable, subtitle="Histogramme de distribution", show_mean=False, multi_index=3)
...
"""
def sns_pairplot(data, hue=None, title=None, palette=None, kind='scatter', diag_kind='auto', indiv_height=2.5):
    sns.set_style("white")
    mult_param = mydefault_plt_parameters()
...
def sns_catplot(x, y, hue, data, title=None, palette=None, kind='strip', order=None, hue_order=None, orient=None, legend=True, figsize=(12,8)):
    height = figsize[1]
    aspect = figsize[0] / figsize[1]
    sns.set_style("white")
    mult_param = mydefault_plt_parameters()
    liste_couleurs = set_colors(False, palette)
    g = sns.catplot(x, y, hue, data, kind=kind, order=order, hue_order=hue_order, orient=orient, height=height, aspect=aspect, legend=legend)
    fig = g.fig
    if title is not None:
        fig.suptitle(title, y=1.05)
    return fig, fig.axes
