...
class ACP:
    ...
    def calculate_acp(self):
        ...
        nb_max_axes =  min(self.data.shape[0] - 1, self.data.shape[1])
        seuils_batons_brises = 1 / np.arange(nb_max_axes, 0 , -1)
        ...
        self.batons_brises["seuils"] = seuils_batons_brises[:self.n_components]
    ...
    def calcul_variables_interpretation(self):
        ...
        for p in np.arange(self.data_scaled.shape[1]):
            corvar[p,:] = corvar[p,:] / np.std(self.data_scaled[:,p], ddof=0)  # division par l'écart type de la variable pour ramener le cercle de corrélation à 1 en cas d'ACP non normée
    ...
    def graphique_correlation_circles(self, axis_ranks=[1,2], nbcol=None,
        ...
        if axis_ranks.ndim > 1:
            ...
            if nbcol is None:
                nbcol = 2
            nblin = nb_graphes // nbcol
            if nb_graphes % nbcol >= 1:
                nblin = nblin + 1
            if figsize is None:
                figsize = (12, (6 * nblin))
        else:
        ...
        if nb_graphes == 1:
            ...
            reste = nb_graphes % nbcol
            if reste >= 1:
                for idax in range(nbcol-reste):
                    graph.ax[nb_graphes-idax-1].set_axis_off()
        graph.fig.tight_layout()
        return graph.fig, graph.ax
